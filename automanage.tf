resource "azapi_resource" "automanage" {
  ...
  name      = var.automanage.config.name
  ...
  body = jsonencode({
    properties = {
      configuration = {
        ...
        "LogAnalytics/Workspace" : var.automanage.config.custom_log_analytics_workspace == true ? azurerm_log_analytics_workspace.default.0.id : null,
...
locals {
  configuration_id = try(replace(azapi_resource.automanage.0.id, "Automanage", "AutoManage"), "/providers/Microsoft.Automanage/bestPractices/AzureBestPracticesProduction")
}